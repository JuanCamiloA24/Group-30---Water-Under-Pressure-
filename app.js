...
function makePdf(lines){const clean=s=>String(s).normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').replace(/[^\\x20-\\x7E]/g,'?').replaceAll('\\','\\\\').replaceAll('(','\\(').replaceAll(')','\\)');const stream=['BT','/F1 14 Tf','50 770 Td',...lines.flatMap((line,i)=>[i?'0 -22 Td':'',`(${clean(line)}) Tj`]),'ET'];const objects=[`<< /Type /Catalog /Pages 2 0 R >>`,`<< /Type /Pages /Kids [3 0 R] /Count 1 >>`,`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 4 0 R >> >> /Contents 5 0 R >>`,`<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>`,`<< /Length ${new TextEncoder().encode(stream.join('\\n')).length} >>\\nstream\\n${stream.join('\\n')}\\nendstream`];const enc=new TextEncoder();let pdf='%PDF-1.4\\n',offsets=[];objects.forEach((o,i)=>{offsets[i]=enc.encode(pdf).length;pdf+=`${i+1} 0 obj\\n${o}\\nendobj\\n`;});const xref=enc.encode(pdf).length;pdf+=`xref\\n0 ${objects.length+1}\\n0000000000 65535 f \\n${offsets.map(o=>String(o).padStart(10,'0')+' 00000 n \\n').join('')}trailer\\n<< /Size ${objects.length+1} /Root 1 0 R >>\\nstartxref\\n${xref}\\n%%EOF`;return new Blob([enc.encode(pdf)],{type:'application/pdf'});}
