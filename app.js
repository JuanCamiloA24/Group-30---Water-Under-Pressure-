...
const nl=String.fromCharCode(10);function makePdf(lines){const clean=s=>String(s).normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').replace(/[^\\x20-\\x7E]/g,'?').replaceAll('\\','\\\\').replaceAll('(','\\(').replaceAll(')','\\)');const stream=['BT','/F1 14 Tf','50 770 Td',...lines.flatMap((line,i)=>[i?'0 -22 Td':'',`(${clean(line)}) Tj`]),'ET'].join(nl);const objects=[`<< /Type /Catalog /Pages 2 0 R >>`,`<< /Type /Pages /Kids [3 0 R] /Count 1 >>`,`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 4 0 R >> >> /Contents 5 0 R >>`,`<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>`,`<< /Length ${new TextEncoder().encode(stream).length} >>${nl}stream${nl}${stream}${nl}endstream`];const enc=new TextEncoder();let pdf='%PDF-1.4'+nl,offsets=[];objects.forEach((o,i)=>{offsets[i]=enc.encode(pdf).length;pdf+=`${i+1} 0 obj${nl}${o}${nl}endobj${nl}`;});const xref=enc.encode(pdf).length;pdf+=`xref${nl}0 ${objects.length+1}${nl}0000000000 65535 f ${nl}${offsets.map(o=>String(o).padStart(10,'0')+' 00000 n '+nl).join('')}trailer${nl}<< /Size ${objects.length+1} /Root 1 0 R >>${nl}startxref${nl}${xref}${nl}%%EOF`;return new Blob([enc.encode(pdf)],{type:'application/pdf'});}
$('#export-report').onclick=()=>{const open=actions.filter(a=>a.status!=='Resolved'),complete=checks.filter(c=>c.state==='Complete').length;const lines=['AquaGuard compliance report',hospitals[activeHospital].name,`Generated: ${new Date().toLocaleString()}`,'',`Monitoring checks complete: ${complete}/${checks.length}`,`Open corrective actions: ${open.length}`,'','Open actions:',...open.map(a=>`- ${a.title} (${a.severity}) - ${a.owner}`)];const url=URL.createObjectURL(makePdf(lines)),link=document.createElement('a');link.href=url;link.download=`aquaguard-${activeHospital}-compliance-report.pdf`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
