...
const nl=String.fromCharCode(10);const pdfText=s=>{let out='';for(const ch of String(s)){const c=ch.charCodeAt(0);out+=c>=32&&c<=126?ch:'?';}return out.replaceAll('\\','\\\\').replaceAll('(','\\(').replaceAll(')','\\)');};function makePdf(lines){const stream=['BT','/F1 14 Tf','50 770 Td',...lines.flatMap((x,i)=>[i?'0 -22 Td':'',`(${pdfText(x)}) Tj`]),'ET'].join(nl),enc=new TextEncoder(),objects=[`<< /Type /Catalog /Pages 2 0 R >>`,`<< /Type /Pages /Kids [3 0 R] /Count 1 >>`,`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 4 0 R >> >> /Contents 5 0 R >>`,`<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>`,`<< /Length ${enc.encode(stream).length} >>${nl}stream${nl}${stream}${nl}endstream`];let pdf='%PDF-1.4'+nl,offsets=[];objects.forEach((o,i)=>{offsets[i]=enc.encode(pdf).length;pdf+=`${i+1} 0 obj${nl}${o}${nl}endobj${nl}`;});const x=enc.encode(pdf).length;pdf+=`xref${nl}0 6${nl}0000000000 65535 f ${nl}${offsets.map(o=>String(o).padStart(10,'0')+' 00000 n '+nl).join('')}trailer${nl}<< /Size 6 /Root 1 0 R >>${nl}startxref${nl}${x}${nl}%%EOF`;return new Blob([enc.encode(pdf)],{type:'application/pdf'});}
$('#export-report').onclick=()=>{const open=actions.filter(a=>a.status!=='Resolved'),complete=checks.filter(c=>c.state==='Complete').length,lines=['AquaGuard compliance report',hospitals[activeHospital].name,`Generated: ${new Date().toLocaleString()}`,'',`Monitoring checks complete: ${complete}/${checks.length}`,`Open corrective actions: ${open.length}`,'','Open actions:',...open.map(a=>`- ${a.title} (${a.severity}) - ${a.owner}`)],url=URL.createObjectURL(makePdf(lines)),link=document.createElement('a');link.href=url;link.download=`aquaguard-${activeHospital}-compliance-report.pdf`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
const publicDataCard=document.createElement('section');publicDataCard.className='card public-data-card';publicDataCard.innerHTML='<p class="eyebrow">PUBLIC DATA · EUROSTAT</p><h2>European water-use context</h2><p class="lead">Live public data adds context to the hospital demo.</p><strong id="eurostat-value">Loading…</strong><p id="eurostat-label">Fetching Spain water data</p><small>Source: Eurostat API · env_wat_abs · Spain</small>';document.querySelector('.context-grid').after(publicDataCard);async function loadEurostatWaterData(){try{const r=await fetch('https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/env_wat_abs?geo=ES&lang=en'),d=await r.json(),v=Object.values(d.value||{}).filter(Number.isFinite);$('#eurostat-value').textContent=v.length?new Intl.NumberFormat('en-GB').format(v[v.length-1]):'Unavailable';$('#eurostat-label').textContent=v.length?'Latest reported water-abstraction value':'Eurostat unavailable';}catch{$('#eurostat-value').textContent='Unavailable';$('#eurostat-label').textContent='Eurostat unavailable';}}
